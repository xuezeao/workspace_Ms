...
int mread_profile_int( const char *section, const char *key,int default_value, 
				const char *file)
{
	char value[32] = {0};
	if(!read_profile_string(section,key,value, sizeof(value),NULL,file))
	{
		...
		sprintf(value, "%d ",default_value);
		//memcpy(b,&a,4);

		if(!write_profile_string(section,key,value,file))
		{
...
int mwrite_profile_int( const char *section, const char *key,int default_value, 
				const char *file)
{
	char value[32] = {0};
	sprintf(value, "%d ",default_value);
